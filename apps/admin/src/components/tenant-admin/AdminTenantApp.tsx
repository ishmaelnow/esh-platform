...
function LedgerPanel({ canManageTenant, summary }: { canManageTenant: boolean; summary: TenantSummary }) {
  ...
  return <section className="panel-stack"><PanelHeader title="Ledger" description={`${operatingCurrency} · immutable balanced postings`} />
    ...
    {workspace === "drivers" ? <div className="table-wrap"><table><thead><tr><th>Driver</th><th>Amount owed</th><th>Onboarding</th><th>Transfers</th><th>Bank payouts</th><th>Updated</th></tr></thead><tbody>{page(filteredPayoutAccounts).map((payout) => { const payable = ledger.accounts.find((account) => account.accountCode === `driver_payable_${payout.driver_profile_id.replaceAll("-", "")}`); const payouts = summary.driverBankPayouts.filter((item) => item.driver_profile_id === payout.driver_profile_id); return <tr key={payout.driver_payout_account_id}><td><strong>{driverName(payout.driver_profile_id)}</strong></td><td>{payable ? formatMinorUnits(payable.balanceMinor * -1, operatingCurrency, fractionDigits) : "—"}</td><td>{payout.onboarding_status.replaceAll("_", " ")}</td><td>{payout.transfers_capability_status ?? "not requested"}</td><td>{payouts.length}</td><td>{formatDate(payout.updated_at)}</td></tr>; })}</tbody></table>{filteredPayoutAccounts.length === 0 ? <EmptyState message="No matching Driver payout accounts." /> : null}</div> : null}

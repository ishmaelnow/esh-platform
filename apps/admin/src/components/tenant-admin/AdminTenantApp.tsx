...
function LedgerPanel({ canManageTenant, summary }: { canManageTenant: boolean; summary: TenantSummary }) {
  ...
  const [message, setMessage] = useState<string | null>(null);
  const [busy, setBusy] = useState(false);
  const [workspace, setWorkspace] = useState<"overview" | "drivers" | "payments" | "payouts" | "journal" | "manual">("overview");
  ...
  const activeCount = workspace === "payments" ? filteredPayments.length : workspace === "drivers" ? filteredPayoutAccounts.length : workspace === "payouts" ? filteredBankPayouts.length : workspace === "journal" ? filteredTransactions.length : 0;
  ...
  return <section className="panel-stack"><PanelHeader title="Ledger" description={`${operatingCurrency} · immutable balanced postings`} />
    ...
    {workspace === "drivers" ? <div className="table-wrap"><table><thead><tr><th>Driver</th><th>Pending</th><th>Collected</th><th>Transferred to Stripe</th><th>Amount owed</th><th>Onboarding</th><th>Transfer capability</th></tr></thead><tbody>{page(filteredPayoutAccounts).map((payout) => { const payable = ledger.accounts.find((account) => account.accountCode === `driver_payable_${payout.driver_profile_id.replaceAll("-", "")}`); const trips = summary.dispatchBookings.filter((booking) => booking.current_driver_profile_id === payout.driver_profile_id && booking.status === "completed" && booking.driver_earnings_minor != null); const paidBookingIds = new Set(summary.riderPaymentAttempts.filter((payment) => payment.status === "paid" && payment.booking_id).map((payment) => payment.booking_id)); const succeededTransfers = summary.driverEarningTransfers.filter((transfer) => transfer.driver_profile_id === payout.driver_profile_id && transfer.status === "succeeded"); const transferredBookingIds = new Set(succeededTransfers.map((transfer) => transfer.booking_id)); const pending = trips.filter((booking) => !paidBookingIds.has(booking.booking_id)).reduce((total, booking) => total + (booking.driver_earnings_minor ?? 0), 0); const collected = trips.filter((booking) => paidBookingIds.has(booking.booking_id) && !transferredBookingIds.has(booking.booking_id)).reduce((total, booking) => total + (booking.driver_earnings_minor ?? 0), 0); const transferred = succeededTransfers.reduce((total, transfer) => total + transfer.amount_minor, 0); return <tr key={payout.driver_payout_account_id}><td><strong>{driverName(payout.driver_profile_id)}</strong></td><td>{formatMinorUnits(pending, operatingCurrency, fractionDigits)}</td><td>{formatMinorUnits(collected, operatingCurrency, fractionDigits)}</td><td>{formatMinorUnits(transferred, operatingCurrency, fractionDigits)}</td><td><strong>{payable ? formatMinorUnits(payable.balanceMinor * -1, operatingCurrency, fractionDigits) : "—"}</strong></td><td>{payout.onboarding_status.replaceAll("_", " ")}</td><td>{payout.transfers_capability_status ?? "not requested"}</td></tr>; })}</tbody></table>{filteredPayoutAccounts.length === 0 ? <EmptyState message="No matching Driver payout accounts." /> : null}</div> : null}
    {workspace === "payments" ? <div className="table-wrap"><table><thead><tr><th>Amount</th><th>Status</th><th>Paid/created</th><th>Booking</th><th>Processor reference</th></tr></thead><tbody>{page(filteredPayments).map((payment) => <tr key={payment.payment_attempt_id}><td><strong>{formatMinorUnits(payment.amount_minor, payment.currency_code, fractionDigits)}</strong></td><td>{payment.status}</td><td>{formatDate(payment.paid_at ?? payment.created_at)}</td><td>{payment.booking_id ? payment.booking_id.slice(0, 8) : "Not finalized"}</td><td><details><summary>View reference</summary><small>{payment.provider_payment_intent_id ?? payment.provider_checkout_session_id}</small></details></td></tr>)}</tbody></table>{filteredPayments.length === 0 ? <EmptyState message="No matching Rider payments." /> : null}</div> : null}
    {workspace === "payouts" ? <div className="table-wrap"><table><thead><tr><th>Driver</th><th>Amount</th><th>Status</th><th>Method</th><th>Created</th><th>Arrival/failure</th></tr></thead><tbody>{page(filteredBankPayouts).map((payout) => <tr key={payout.driver_bank_payout_id}><td>{driverName(payout.driver_profile_id)}</td><td><strong>{formatMinorUnits(payout.amount_minor, payout.currency_code, fractionDigits)}</strong></td><td>{payout.status.replaceAll("_", " ")}</td><td>{payout.automatic ? "Automatic" : "Manual"} · {payout.method ?? "standard"}</td><td>{formatDate(payout.provider_created_at)}</td><td>{payout.failure_message ?? payout.failure_code ?? (payout.expected_arrival_at ? formatDate(payout.expected_arrival_at) : "—")}</td></tr>)}</tbody></table>{filteredBankPayouts.length === 0 ? <EmptyState message="No matching connected-account bank payouts." /> : null}</div> : null}
    {workspace === "journal" ? <div>{page(filteredTransactions).map((transaction) => <article className="data-card" key={transaction.transactionId}><div><strong>{transaction.description}</strong><p>{formatDate(transaction.effectiveAt)} · {(transaction.externalKey.split(":", 1)[0] ?? "transaction").replaceAll("_", " ")}</p><details><summary>View balanced entries and references</summary>{transaction.entries.map((entry) => <small key={`${transaction.transactionId}-${entry.accountCode}-${entry.side}`}>{entry.side.toUpperCase()} {entry.accountCode}: {formatMinorUnits(entry.amountMinor, operatingCurrency, fractionDigits)}<br /></small>)}<small>{transaction.externalKey}{transaction.bookingId ? ` · Booking ${transaction.bookingId}` : ""}</small></details></div></article>)}{filteredTransactions.length === 0 ? <EmptyState message="No matching journal transactions." /> : null}</div> : null}
    {workspace === "manual" ? <form className="form-grid" onSubmit={(event) => void postJournal(event)}><h4>Post manual journal</h4><p className="empty-state">Manual entries are permanent. Use them only for authorized corrections or operating adjustments; never delete or rewrite posted history.</p><label>Description<input name="description" required maxLength={240} placeholder="Reason for this journal" /></label><label>Amount ({operatingCurrency})<input name="amount" required inputMode="decimal" placeholder="10.00" /></label><label>Debit account<select name="debitAccount" required defaultValue="cash_clearing">{ledger.accounts.map((account) => <option key={account.accountId} value={account.accountCode}>{account.accountName}</option>)}</select></label><label>Credit account<select name="creditAccount" required defaultValue="platform_fees">{ledger.accounts.map((account) => <option key={account.accountId} value={account.accountCode}>{account.accountName}</option>)}</select></label><button disabled={!canManageTenant || busy} type="submit">Post immutable balanced transaction</button></form> : null}

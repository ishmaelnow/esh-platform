...
function LedgerPanel({ canManageTenant, summary }: { canManageTenant: boolean; summary: TenantSummary }) {
  ...
  return <section className="panel-stack"><PanelHeader title="Ledger" description={`${operatingCurrency} · immutable balanced postings`} />
    ...
    <div><h4>Rider payments</h4>{summary.riderPaymentAttempts.length === 0 ? <EmptyState message="No Rider payment attempts have been recorded." /> : summary.riderPaymentAttempts.map((payment) => <article className="data-card" key={payment.payment_attempt_id}><div><strong>{formatMinorUnits(payment.amount_minor, payment.currency_code, fractionDigits)} · {payment.status}</strong><p>{formatDate(payment.paid_at ?? payment.created_at)} · Stripe Checkout</p><small>Quote {payment.quote_id}{payment.booking_id ? ` · Booking ${payment.booking_id}` : " · Booking not finalized"}</small>{payment.failure_message ? <p>{payment.failure_message}</p> : null}</div></article>)}</div>
    <div><h4>Driver payout accounts</h4>{summary.driverPayoutAccounts.length === 0 ? <EmptyState message="No Driver has started Stripe payout onboarding." /> : summary.driverPayoutAccounts.map((payout) => { const driver = summary.drivers.find((item) => item.driver_profile_id === payout.driver_profile_id); return <article className="data-card" key={payout.driver_payout_account_id}><div><strong>{driver?.display_name ?? "Driver"} · {payout.onboarding_status.replaceAll("_", " ")}</strong><p>Transfers: {payout.transfers_capability_status ?? "not requested"} · Payouts: {payout.payouts_enabled ? "enabled" : "not enabled"}</p><small>{payout.requirements_currently_due.length} requirement(s) currently due · Updated {formatDate(payout.updated_at)}</small></div></article>; })}</div>
    <div><h4>Driver bank payouts</h4>{summary.driverBankPayouts.length === 0 ? <EmptyState message="No connected-account bank payouts have been reported by Stripe." /> : summary.driverBankPayouts.map((payout) => { const driver = summary.drivers.find((item) => item.driver_profile_id === payout.driver_profile_id); return <article className="data-card" key={payout.driver_bank_payout_id}><div><strong>{driver?.display_name ?? "Driver"} · {formatMinorUnits(payout.amount_minor, payout.currency_code, fractionDigits)} · {payout.status.replaceAll("_", " ")}</strong><p>{payout.automatic ? "Automatic" : "Manual"} {payout.method ?? "standard"} payout · {formatDate(payout.provider_created_at)}</p>{payout.expected_arrival_at ? <small>Expected arrival {formatDate(payout.expected_arrival_at)}</small> : null}{payout.failure_message ? <p>{payout.failure_message}</p> : payout.failure_code ? <p>{payout.failure_code}</p> : null}</div></article>; })}</div>
    <form className="form-grid" onSubmit={(event) => void postJournal(event)}><h4>Post manual journal</h4><p className="empty-state">For the verification entry, record money received by debiting Cash and payment clearing and crediting Platform fee revenue. Both sides must use the same amount.</p><label>Description<input name="description" required maxLength={240} placeholder="LEDGER V1 MANUAL TEST" /></label><label>Amount ({operatingCurrency})<input name="amount" required inputMode="decimal" placeholder="10.00" /></label><label>Debit account<select name="debitAccount" required defaultValue="cash_clearing">{ledger.accounts.map((account) => <option key={account.accountId} value={account.accountCode}>{account.accountName}</option>)}</select></label><label>Credit account<select name="creditAccount" required defaultValue="platform_fees">{ledger.accounts.map((account) => <option key={account.accountId} value={account.accountCode}>{account.accountName}</option>)}</select></label><button disabled={!canManageTenant || busy} type="submit">Post balanced transaction</button></form>

...
import { farePolicyLabel } from "@/lib/tenant-admin/fare-policy";
import {
  formatMinorUnits,
...
function PricingPanel({ canManageTenant, onRefresh, summary }: { canManageTenant: boolean; onRefresh: () => void; summary: TenantSummary }) {
  ...
  const [farePolicy, setFarePolicy] = useState(settings?.fare_policy ?? "guaranteed_upfront");
  const [protectedCapMode, setProtectedCapMode] = useState(settings?.protected_cap_mode ?? "percentage");
  async function save(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); const form = new FormData(event.currentTarget);
    const value = (name: string) => { const entry = form.get(name); return typeof entry === "string" ? entry : ""; };
    setBusy(true); setMessage(null);
    try {
      const protectedCapValue = protectedCapMode === "percentage"
        ? Math.round(Number(value("protectedCap")) * 100)
        : parseMoneyToMinorUnits(value("protectedCap"), 2, true);
      if (!Number.isFinite(protectedCapValue) || protectedCapValue < 0)
        throw new Error("Protected fare cap must be a nonnegative value.");
      const result = await supabase.rpc("set_tenant_pricing_and_fare_policy", {
        ...
        fare_policy_value: farePolicy,
        protected_cap_mode_value: protectedCapMode,
        protected_cap_value_value: protectedCapValue,
      });
  ...
  return <section className="panel-stack"><PanelHeader title="Trip pricing" description="Configure route-based prices and the fare contract Riders accept before payment." />
    ...
    <form className="form-grid" onSubmit={(event) => void save(event)}>
      ...
      <h4>Fare contract</h4>
      <label>Fare policy<select name="farePolicy" value={farePolicy} onChange={(event) => setFarePolicy(event.target.value)}>
        <option value="guaranteed_upfront">{farePolicyLabel("guaranteed_upfront")}</option>
        <option value="protected_flexible">{farePolicyLabel("protected_flexible")}</option>
        <option value="metered_actual">{farePolicyLabel("metered_actual")}</option>
      </select></label>
      <p className="empty-state">{farePolicy === "guaranteed_upfront"
        ? "The quoted fare remains final through ordinary traffic and route changes. Actual route metrics are retained only for operational comparison."
        : farePolicy === "metered_actual"
          ? "The quote is an estimate. The final fare uses trusted actual trip time and mileage, and any difference requires reviewed settlement."
          : "The final fare uses trusted actual trip time and mileage, may decrease, and cannot exceed the disclosed cap without a separately approved trip change."}</p>
      {farePolicy === "protected_flexible" ? <>
        <label>Maximum increase type<select name="protectedCapMode" value={protectedCapMode} onChange={(event) => setProtectedCapMode(event.target.value)}>
          <option value="percentage">Percentage</option><option value="fixed">Fixed amount</option>
        </select></label>
        <label>{protectedCapMode === "percentage" ? "Maximum increase (%)" : `Maximum increase (${currency})`}
          <input defaultValue={protectedCapMode === "percentage" ? ((settings?.protected_cap_mode === "percentage" ? settings.protected_cap_value : 2000) / 100).toFixed(2) : display(settings?.protected_cap_mode === "fixed" ? settings.protected_cap_value : undefined, "10.00")} min="0" max={protectedCapMode === "percentage" ? "100" : undefined} name="protectedCap" step="0.01" type="number" required />
        </label>
      </> : null}
      ...
      <p className="empty-state">Fare = base + road miles × per-mile rate + traffic-aware route minutes × per-minute rate, subject to the minimum fare. The quote and its fare contract are immutable after booking.</p>
...
function LedgerPanel({ canManageTenant, onRefresh, summary }: { canManageTenant: boolean; onRefresh: () => void; summary: TenantSummary }) {
  ...
  const [message, setMessage] = useState<string | null>(null);
  const [busy, setBusy] = useState(false);
  const [workspace, setWorkspace] = useState<"overview" | "drivers" | "payments" | "wallets" | "disputes" | "payouts" | "reconciliations" | "journal" | "manual">("overview");
  ...
  async function reviewFareReconciliation(reconciliationId: string, decision: "approved" | "rejected") {
  ...
  const reconciliationByBooking = new Map(summary.tripFareReconciliations.map((item) => [item.booking_id, item]));
  const reconciliationRows = summary.dispatchBookings
    .filter((booking) => booking.status === "completed")
    .map((booking) => reconciliationByBooking.get(booking.booking_id) ?? {
      reconciliation_id: `booking:${booking.booking_id}`,
      booking_id: booking.booking_id,
      created_at: booking.completed_at ?? booking.updated_at,
      quoted_distance_meters: booking.route_distance_meters,
      actual_distance_meters: null,
      quoted_duration_seconds: booking.route_duration_seconds,
      actual_duration_seconds: null,
      quoted_fare_minor: booking.final_fare_minor ?? booking.estimated_fare_minor,
      calculated_fare_minor: null,
      raw_calculated_fare_minor: null,
      fare_policy: null,
      maximum_fare_minor: null,
      adjustment_minor: null,
      currency_code: booking.fare_currency_code ?? operatingCurrency,
      source: "none",
      status: "no_metrics",
      review_note: null,
    });
  const filteredReconciliations = reconciliationRows.filter((item) => !search || [item.status, item.source, item.booking_id].some((value) => value?.toLowerCase().includes(search)));
  ...
  const activeCount = workspace === "payments" ? filteredPayments.length : workspace === "wallets" ? summary.riderWalletEntries.length : workspace === "disputes" ? filteredDisputes.length : workspace === "drivers" ? filteredPayoutAccounts.length : workspace === "payouts" ? filteredBankPayouts.length : workspace === "reconciliations" ? filteredReconciliations.length : workspace === "journal" ? filteredTransactions.length : 0;
  ...
  return <section className="panel-stack"><PanelHeader title="Ledger" description={`${operatingCurrency} · immutable balanced postings`} />
    ...
    <nav aria-label="Ledger workspace" className="ledger-workspace-nav">{([['overview','Overview'],['drivers','Driver balances'],['payments','Rider payments'],['wallets','Rider wallets'],['disputes','Disputes'],['payouts','Bank payouts'],['reconciliations','Fare reconciliation'],['journal','Journal'],['manual','Manual journal']] as const).map(([key, label]) => <button className={workspace === key ? "primary-button" : "secondary-button"} key={key} onClick={() => changeWorkspace(key)} type="button">{label}</button>)}</nav>
    ...
    {workspace === "reconciliations" ? <div className="table-wrap"><p className="empty-state">Completed trips are listed here. Trusted route comparisons are evaluated under the immutable fare contract accepted before payment; normal route choice is never locked.</p><table><thead><tr><th>Booking</th><th>Policy</th><th>Quoted route</th><th>Actual route</th><th>Quoted fare</th><th>Actual meter</th><th>Contract fare</th><th>Adjustment</th><th>Status</th><th>Review</th></tr></thead><tbody>{page(filteredReconciliations).map((item) => <tr key={item.reconciliation_id}><td><strong>{item.booking_id.slice(0, 8)}</strong><br /><small>{formatDate(item.created_at)}</small></td><td>{item.fare_policy ? item.fare_policy.replaceAll("_", " ") : "Legacy locked fare"}{item.maximum_fare_minor != null ? <><br /><small>Cap {formatMinorUnits(item.maximum_fare_minor, item.currency_code, fractionDigits)}</small></> : null}</td><td>{item.quoted_distance_meters != null && item.quoted_duration_seconds != null ? `${(item.quoted_distance_meters / 1609.344).toFixed(1)} mi · ${Math.round(item.quoted_duration_seconds / 60)} min` : "—"}</td><td>{item.actual_distance_meters != null && item.actual_duration_seconds != null ? `${(item.actual_distance_meters / 1609.344).toFixed(1)} mi · ${Math.round(item.actual_duration_seconds / 60)} min` : "Not captured"}</td><td>{item.quoted_fare_minor != null ? formatMinorUnits(item.quoted_fare_minor, item.currency_code, fractionDigits) : "—"}</td><td>{item.raw_calculated_fare_minor != null ? formatMinorUnits(item.raw_calculated_fare_minor, item.currency_code, fractionDigits) : item.calculated_fare_minor != null ? formatMinorUnits(item.calculated_fare_minor, item.currency_code, fractionDigits) : "—"}</td><td>{item.calculated_fare_minor != null ? formatMinorUnits(item.calculated_fare_minor, item.currency_code, fractionDigits) : "—"}</td><td><strong>{item.adjustment_minor == null ? "—" : item.adjustment_minor === 0 ? "No change" : `${item.adjustment_minor > 0 ? "+" : "−"}${formatMinorUnits(Math.abs(item.adjustment_minor), item.currency_code, fractionDigits)}`}</strong></td><td>{item.status === "no_metrics" ? "No trusted route metrics" : item.status.replaceAll("_", " ")}</td><td>{item.status === "pending_review" ? <><button className="secondary-button" disabled={!canManageTenant || busy} onClick={() => void reviewFareReconciliation(item.reconciliation_id, "approved")} type="button">Approve</button> <button className="danger-button" disabled={!canManageTenant || busy} onClick={() => void reviewFareReconciliation(item.reconciliation_id, "rejected")} type="button">Reject</button></> : item.review_note ?? "—"}</td></tr>)}</tbody></table>{filteredReconciliations.length === 0 ? <EmptyState message="No completed trips found." /> : null}</div> : null}

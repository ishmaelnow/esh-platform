...
function LedgerPanel({ canManageTenant, summary }: { canManageTenant: boolean; summary: TenantSummary }) {
  ...
  return <section className="panel-stack"><PanelHeader title="Ledger" description={`${operatingCurrency} · immutable balanced postings`} />
    ...
    {workspace === "payments" ? <div className="table-wrap"><table><thead><tr><th>Amount</th><th>Status</th><th>Paid/created</th><th>Booking</th><th>Refund</th><th>Processor reference</th></tr></thead><tbody>{page(filteredPayments).map((payment) => { const refund = summary.riderPaymentRefunds.find((item) => item.payment_attempt_id === payment.payment_attempt_id); return <tr key={payment.payment_attempt_id}><td><strong>{formatMinorUnits(payment.amount_minor, payment.currency_code, fractionDigits)}</strong></td><td>{payment.status}</td><td>{formatDate(payment.paid_at ?? payment.created_at)}</td><td>{payment.booking_id ? payment.booking_id.slice(0, 8) : "Not finalized"}</td><td>{refund ? `${formatMinorUnits(refund.amount_minor, refund.currency_code, fractionDigits)} · ${refund.status}` : "—"}</td><td><details><summary>View reference</summary><small>{payment.provider_payment_intent_id ?? payment.provider_checkout_session_id}</small></details></td></tr>; })}</tbody></table>{filteredPayments.length === 0 ? <EmptyState message="No matching Rider payments." /> : null}</div> : null}

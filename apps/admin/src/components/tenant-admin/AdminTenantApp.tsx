...
function LedgerPanel({ canManageTenant, onRefresh, summary }: { canManageTenant: boolean; onRefresh: () => void; summary: TenantSummary }) {
  ...
  const [message, setMessage] = useState<string | null>(null);
  const [busy, setBusy] = useState(false);
  const [workspace, setWorkspace] = useState<"overview" | "drivers" | "payments" | "disputes" | "payouts" | "journal" | "manual">("overview");
  ...
  const activeCount = workspace === "payments" ? filteredPayments.length : workspace === "disputes" ? filteredDisputes.length : workspace === "drivers" ? filteredPayoutAccounts.length : workspace === "payouts" ? filteredBankPayouts.length : workspace === "journal" ? filteredTransactions.length : 0;
  ...
  return <section className="panel-stack"><PanelHeader title="Ledger" description={`${operatingCurrency} · immutable balanced postings`} />
    ...
    {workspace === "disputes" ? <div className="table-wrap"><table><thead><tr><th>Amount</th><th>Status</th><th>Reason</th><th>Booking</th><th>Funds</th><th>Evidence due</th><th>Processor reference</th></tr></thead><tbody>{page(filteredDisputes).map((dispute) => { const transfer = summary.driverEarningTransfers.find((item) => item.booking_id === dispute.booking_id && item.status === "succeeded"); return <tr key={dispute.rider_payment_dispute_id}><td><strong>{formatMinorUnits(dispute.amount_minor, dispute.currency_code, fractionDigits)}</strong></td><td>{dispute.status.replaceAll("_", " ")}</td><td>{dispute.reason.replaceAll("_", " ")}</td><td>{dispute.booking_id?.slice(0, 8) ?? "Not finalized"}</td><td>{dispute.funds_reinstated_at ? `Reinstated ${formatDate(dispute.funds_reinstated_at)}` : dispute.funds_withdrawn_at ? `Withdrawn ${formatDate(dispute.funds_withdrawn_at)}${transfer ? " · Driver transfer requires reviewed recovery" : ""}` : "Not withdrawn"}</td><td>{dispute.evidence_due_at ? formatDate(dispute.evidence_due_at) : "—"}</td><td><details><summary>View reference</summary><small>{dispute.provider_dispute_id}</small></details></td></tr>; })}</tbody></table>{filteredDisputes.length === 0 ? <EmptyState message="No matching Stripe disputes." /> : null}</div> : null}

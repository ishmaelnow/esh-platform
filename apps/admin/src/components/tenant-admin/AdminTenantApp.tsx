    ...
    <section className="content-stack">
      ...
      {summary.riderBookingSeries.length > 0 ? <section className="panel">
        ...
        <div className="table-wrap"><table><thead><tr><th>Rider</th><th>Route</th><th>Pattern</th><th>Occurrences</th><th>Status</th></tr></thead><tbody>
          {summary.riderBookingSeries.map((series) => { const rider = summary.riderProfiles.find((item) => item.rider_profile_id === series.rider_profile_id); const occurrences = summary.riderBookingSeriesOccurrences.filter((item) => item.rider_booking_series_id === series.rider_booking_series_id); const awaiting = occurrences.filter((item) => item.status === "awaiting_payment").length; const pending = occurrences.filter((item) => item.status === "payment_pending").length; const booked = occurrences.filter((item) => item.status === "booked").length; return <tr key={series.rider_booking_series_id}><td><strong>{rider?.display_name ?? series.rider_profile_id.slice(0, 8)}</strong></td><td>{series.pickup_address}<br />to {series.destination_address}</td><td>{series.weekdays.map((day) => ["","Mon","Tue","Wed","Thu","Fri","Sat","Sun"][day]).join(", ")} · {series.local_pickup_time.slice(0, 5)}<br />{series.start_date} through {series.end_date}</td><td>{booked} booked · {pending} payment pending · {awaiting} awaiting · {occurrences.length} total</td><td>{series.status}</td></tr>; })}
        </tbody></table></div>
      </section> : null}

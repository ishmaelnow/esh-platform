...
function LedgerPanel({ canManageTenant, onRefresh, summary }: { canManageTenant: boolean; onRefresh: () => void; summary: TenantSummary }) {
  ...
  const filteredReconciliations = summary.tripFareReconciliations.filter((item) => !search || [item.status, item.source, item.booking_id].some((value) => value?.toLowerCase().includes(search)));
  ...
  return <section className="panel-stack"><PanelHeader title="Ledger" description={`${operatingCurrency} · immutable balanced postings`} />
    ...
    {workspace === "reconciliations" ? <div className="table-wrap"><p className="empty-state">These are trusted post-trip comparisons. The locked fare and financial records are unchanged until an explicit settlement action is completed.</p><table><thead><tr><th>Booking</th><th>Quoted route</th><th>Actual route</th><th>Locked fare</th><th>Calculated fare</th><th>Adjustment</th><th>Status</th><th>Review</th></tr></thead><tbody>{page(filteredReconciliations).map((item) => <tr key={item.reconciliation_id}><td><strong>{item.booking_id.slice(0, 8)}</strong><br /><small>{formatDate(item.created_at)}</small></td><td>{(item.quoted_distance_meters / 1609.344).toFixed(1)} mi · {Math.round(item.quoted_duration_seconds / 60)} min</td><td>{(item.actual_distance_meters / 1609.344).toFixed(1)} mi · {Math.round(item.actual_duration_seconds / 60)} min</td><td>{formatMinorUnits(item.quoted_fare_minor, item.currency_code, fractionDigits)}</td><td>{formatMinorUnits(item.calculated_fare_minor, item.currency_code, fractionDigits)}</td><td><strong>{item.adjustment_minor === 0 ? "—" : `${item.adjustment_minor > 0 ? "+" : "−"}${formatMinorUnits(Math.abs(item.adjustment_minor), item.currency_code, fractionDigits)}`}</strong></td><td>{item.status.replaceAll("_", " ")}</td><td>{item.status === "pending_review" ? <><button className="secondary-button" disabled={!canManageTenant || busy} onClick={() => void reviewFareReconciliation(item.reconciliation_id, "approved")} type="button">Approve</button> <button className="danger-button" disabled={!canManageTenant || busy} onClick={() => void reviewFareReconciliation(item.reconciliation_id, "rejected")} type="button">Reject</button></> : item.review_note ?? "—"}</td></tr>)}</tbody></table>{filteredReconciliations.length === 0 ? <EmptyState message="No completed trips have trusted fare comparisons yet." /> : null}</div> : null}

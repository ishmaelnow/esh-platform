...
export type WorkspaceSummary = {
  ...
  entitlementStatus: ProductEntitlementStatus;
  roles: WorkspaceRoleKey[];
};
...
export type WorkspaceEnrollment = {
  enrollmentId: string;
  membershipId: string;
  workspaceKey: ProductWorkspaceKey;
  status: string;
...
export function parseWorkspaceAdminSnapshot(value: Json): WorkspaceAdminSnapshot {
  ...
  return {
    ...
    workspaces: asArray(source.workspaces).flatMap((item) => {
      ...
      return [
        {
          workspaceKey: asString(row.workspace_key) as ProductWorkspaceKey,
          displayName: asString(row.display_name),
          description: asString(row.description),
          status: parseStatus(row.status),
          entitlementStatus: parseEntitlementStatus(row.entitlement_status),
...
export function rolesForWorkspace(workspaceKey: ProductWorkspaceKey) {
  return workspaceKey === "transportation"
    ? (["transportation_admin"] as const)
    : ([
        "community_member",
        "community_admin",
        "community_moderator",
...
function parseRoles(value: unknown) {
  return asArray(value)
    .map(asString)
    .filter((role): role is WorkspaceRoleKey => roleKeys.has(role));
}
function parseStatus(value: unknown): WorkspaceSummary["status"] {
  return value === "enabled" || value === "suspended" ? value : "disabled";
}
function parseEntitlementStatus(value: unknown): ProductEntitlementStatus {
  return value === "suspended" || value === "revoked" ? value : "granted";
}

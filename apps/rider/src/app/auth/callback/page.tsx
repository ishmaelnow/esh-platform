...
export default function RiderAuthCallbackPage() {
  ...
  useEffect(() => {
    if (!supabase) {
      setError("Rider sign-in is not configured. Please try again later.");
      return;
    }

    let cancelled = false;
    const callback = new URL(window.location.href);
    const tenant = callback.searchParams.get("tenant");
    const destination = tenant ? `/?tenant=${encodeURIComponent(tenant)}` : "/";
    ...
    const complete = async () => {
      ...
      if (!cancelled) {
        setStatus("Signed in. Redirecting to ESH Rider…");
  ...
  return (
    <main className="auth-callback-page">
      <section className="auth-callback-card" aria-live="polite">
        ...
        {error ? <Link href="/">Return to Rider sign-in</Link> : null}
      </section>
    </main>
  );
}
